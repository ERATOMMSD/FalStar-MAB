function [val__, time_values__] = My_STL_EvalThom(Sys, phi, signalID, P, trajs, t)
...
for ii=1:numTrajs % we loop on every traj in case we check more than one
    ...
    if exist('t','var')
        ...
        [val, time_values] = GetValues(Sys, phi, signalID, Pii, traj, interval);
        ...
        [val__ii, time_values__ii] = GetValues(Sys, phi,signalID, Pii, traj, interval);
...
function [valarray, time_values] = GetValues(Sys, phi, signalID, P, traj, interval)
global BreachGlobOpt;
...
switch(phi.type)
    ...
    case 'not'
        [valarray, time_values] = GetValues(Sys, phi.phi, signalID, P, traj, interval);
        valarray = - valarray;
        
    case 'or'
        [valarray1, time_values1] = GetValues(Sys, phi.phi1, signalID, P, traj, interval);
        [valarray2, time_values2] = GetValues(Sys, phi.phi2, signalID, P, traj, interval);
        [time_values, valarray] = MyRobustOr(time_values1, valarray1, time_values2, valarray2, signalID);
        
    case 'and'
        [valarray1, time_values1] = GetValues(Sys, phi.phi1, signalID, P, traj, interval);
        [valarray2, time_values2] = GetValues(Sys, phi.phi2, signalID, P, traj, interval);
        [time_values, valarray] = MyRobustAnd(time_values1, valarray1, time_values2, valarray2, signalID);
    ...
    case 'andn'
        ...
        for ii=1:n_phi
            [valarray{ii},time_values{ii}] = GetValues(Sys, phi.phin(ii), signalID, P, traj, interval);
        end
    ...
    case 'always'
        ...
        [valarray, time_values] = GetValues(Sys, phi.phi, signalID, P, traj, next_interval);
    ...
    case 'av_eventually'
        ...
        I___(1) = min(I___(1), I___(2));
        next_interval = I___+interval;
        [valarray1, time_values1] = GetValues(Sys, phi.phi, signalID, P, traj, next_interval);
        if(I___(end)~=inf)
    ...
    case 'until'
        ...
        [valarray1, time_values1] = GetValues(Sys, phi.phi1, signalID, P, traj, interval1);
        [valarray2, time_values2] = GetValues(Sys, phi.phi2, signalID, P, traj, interval2);
        if(I___(end)~=inf)
            time_values1 = [time_values1 time_values1(end)+I___(end)];
            valarray1 = [valarray1 valarray1(end)];
...
function [time_values, valarray] = MyRobustAnd(time_values1, valarray1, time_values2, valarray2, signalID)
    if signalID == 1
...
function [time_values, valarray] = MyRobustOr(time_values1, valarray1, time_values2, valarray2, signalID)
    global rob_pre;
    ...
    if signalID == 1
        ...
        while i1 <= length(time_values1) && i2 <= length(time_values2) %works only when leng(t1) = leng(t2)
            
            if time_values1(i1) > time_values2(i2)
                i2 = i2 + 1;
            elseif time_values1(i1) < time_values2(i2)
                i1 = i1 + 1;
            else
                if valarray2(i2) < 0
                    valarray = [valarray valarray1(i1)];
                    time_values = [time_values time_values1(i1)];
        ...
        while i1 <= length(time_values1) && i2 <= length(time_values2) %works only when leng(t1) = leng(t2)
            ...
            elseif time_values1(i1) < time_values2(i2)
                ...
                if valarray1(i1) < 0
                    valarray = [valarray valarray2(i2)];
                    time_values = [time_values time_values1(i1)];
      ...
      %  if isempty(time_values)
      %      preprocess_status = 1;
            %preprocess.robustness = min(valarray1);
      %      valarray = min(valarray1);
      %      time_values = 0;
      %  end
       rob_pre = min(valarray1);
    end
